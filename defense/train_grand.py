...
from defense.pygcn.utils import load_data, accuracy, sparse_mx_to_torch_sparse_tensor, load_data1
from defense.pygcn.models import GCN, MLP
...
parser.add_argument('--input_droprate', type=float, default=0.5, # 0.4
...
parser.add_argument('--dropnode_rate', type=float, default=0,
                    help='Dropnode rate (1 - keep probability).')
parser.add_argument('--patience', type=int, default=100, help='Patience')
parser.add_argument('--order', type=int, default=10, help='Propagation step')
parser.add_argument('--sample', type=int, default=1, help='Sampling times of dropnode')
parser.add_argument('--tem', type=float, default=0.5, help='Sharpening temperature')
parser.add_argument('--lam', type=float, default=0., help='Lamda')
...
def train_grand(data):

    A, features, labels, idx_train, idx_val, idx_test = load_data1(data)
    ...
    for epoch in range(args.epochs):
        ...
        l, a = train(epoch, model, optimizer, A, labels, features, idx_train, idx_val)
    ...
    test(model, A, features, labels, idx_test)
...
def train(epoch, model, optimizer, A, labels, features, idx_train, idx_val):
    t = time.time()
    ...
    loss_train = 0.
    for k in range(K):
        # print(F.nll_loss(output_list[k][idx_train], labels[idx_train]))
...
def test(model, A, features, labels, idx_test):
    model.eval()
    X = features
    X = rand_prop(X, A, training=False)
    output = model(X)
